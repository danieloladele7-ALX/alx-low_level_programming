 ...
 * @elf_content: A pointer to an array containing the
 * ELF content (for the ELF magic numbers).
 *
 * Return: Void or exit code 98 on failure.
 */
void check_file(unsigned char *elf_content)
{
	int idx;

	for (idx = 0; idx < 4; idx++)
	{
		if (elf_content[idx] != 127 && elf_content[idx] != 'E' &&
		    elf_content[idx] != 'L' && elf_content[idx] != 'F')
		{
 ...
 * @vr: A pointer to an array containing the ELF version.
 *
 * Return: void.
 */
void print_version(unsigned char *vr)
{
	printf("  Version:                           %d",
	       vr[EI_VERSION]);

	switch (vr[EI_VERSION])
	{
	case EV_CURRENT:
		printf(" (current)\n");
		break;
 ...
 * @os_a: A pointer to an array containing the ELF version.
 *
 * Return: void.
 */
void print_osabi(unsigned char *os_a)
{
	printf("  OS/ABI:                            ");

	switch (os_a[EI_OSABI])
	{
	case ELFOSABI_NONE:
...
int main(int ac, char **av)
{
	...
	if (!header)
	{
		dprintf(STDERR_FILENO, "Error: No memory allocated for %s\n",
			av[1]);
		close_file(openn), exit(98);
	}
	readd = read(openn, header, sizeof(Elf64_Ehdr));
	if (readd == -1)
	{
		printf("Error: Cannot read file %s\n", av[1]);
		free(header), close_file(openn), exit(98);
	}
